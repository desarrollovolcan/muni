...
<head>
    ...
    <style>
        html,body,#publicProjectMap{height:100%;margin:0;width:100%}body{background:#eef3f8;color:#263548;font-family:Inter,Arial,sans-serif;overflow:hidden}.screen-map{height:100vh;position:relative;width:100vw}#publicProjectMap{background:#dbe6f2}.project-sidebar{background:rgba(255,255,255,.94);border:1px solid rgba(214,224,238,.9);border-radius:0 22px 22px 0;box-shadow:0 18px 55px rgba(31,54,86,.18);display:flex;flex-direction:column;height:calc(100vh - 28px);left:0;overflow:hidden;position:absolute;top:14px;width:360px;z-index:600}.sidebar-head{border-bottom:1px solid #e6edf7;padding:16px 18px 14px}.sidebar-brand{align-items:center;display:flex;gap:12px}.sidebar-brand img{height:42px;max-width:145px;object-fit:contain}.sidebar-brand h1{color:#25364d;font-size:17px;line-height:1.1;margin:0}.sidebar-brand p{color:#74849a;font-size:11px;margin:4px 0 0}.screen-counter{background:#2d80ff;border-radius:999px;color:#fff;font-size:12px;font-weight:800;margin-left:auto;padding:6px 10px}.project-list-title{align-items:center;color:#516177;display:flex;font-size:12px;font-weight:800;justify-content:space-between;margin-top:14px}.public-project-list{display:grid;gap:12px;overflow:auto;padding:14px}.public-project-card{background:#fff;border:1px solid #e3ebf5;border-radius:16px;box-shadow:0 10px 28px rgba(31,54,86,.08);cursor:pointer;overflow:hidden;text-align:left;transition:.18s ease;width:100%}.public-project-card:hover,.public-project-card.active{border-color:#9fc7ff;box-shadow:0 16px 32px rgba(45,128,255,.16);transform:translateY(-1px)}.public-project-card .map-slider img{border-radius:14px 14px 0 0;height:148px!important;margin:0}.public-card-body{padding:14px}.public-card-title{display:block;color:#25364d;font-size:15px;font-weight:850;line-height:1.18;margin:6px 0 8px}.public-card-meta{display:block;color:#63758d;font-size:12px;line-height:1.45}.tag{border-radius:999px;display:inline-flex;font-size:10px;font-weight:850;padding:5px 9px}.tag.en-ejecucion{background:#e7fbef;color:#138f51}.tag.finalizado{background:#e7f1ff;color:#2d80ff}.tag.planificacion,.tag.licitacion{background:#fff3da;color:#d28400}.tag.pausado{background:#fff0ef;color:#d43f3a}.progress{display:block;background:#e8eef7;border-radius:999px;height:8px;margin-top:11px;overflow:hidden}.progress span{background:linear-gradient(90deg,#58d9c7,#2d80ff);display:block;height:100%}.screen-legend{background:rgba(255,255,255,.92);border:1px solid rgba(214,224,238,.88);border-radius:14px;bottom:24px;box-shadow:0 18px 45px rgba(31,54,86,.12);display:flex;gap:14px;left:388px;padding:11px 14px;position:absolute;z-index:500}.screen-legend span{align-items:center;color:#506176;display:flex;font-size:12px;font-weight:700;gap:6px}.dot{border-radius:50%;display:inline-block;height:9px;width:9px}.map-slider{display:block;position:relative}.map-slider img{background:#f3f6fa;border-radius:12px;height:150px;margin-bottom:10px;object-fit:cover;width:100%}.slider-btn{background:rgba(31,54,86,.74);border:0;border-radius:50%;color:#fff;display:grid;font-size:16px;height:28px;place-items:center;position:absolute;top:50%;transform:translateY(-50%);width:28px;z-index:2}.slider-btn.prev{left:8px}.slider-btn.next{right:8px}.slider-dots{bottom:10px;display:flex;gap:5px;justify-content:center;left:0;position:absolute;right:0}.slider-dots i{background:rgba(255,255,255,.78);border-radius:50%;height:6px;width:6px}.leaflet-popup-content-wrapper{border-radius:16px}.leaflet-popup-content{min-width:280px}.popup-title{color:#27384f;font-size:15px;font-weight:850;line-height:1.2;margin-bottom:7px}.popup-meta{color:#63758d;font-size:12px;line-height:1.5}.empty-message{background:rgba(255,255,255,.95);border:1px solid #dce6f2;border-radius:18px;box-shadow:0 18px 45px rgba(31,54,86,.12);left:calc(50% + 180px);padding:22px;position:absolute;text-align:center;top:50%;transform:translate(-50%,-50%);z-index:600}@media(max-width:900px){.project-sidebar{border-radius:0;height:44vh;top:0;width:100%;}.public-project-list{grid-auto-flow:column;grid-auto-columns:285px;overflow-x:auto;overflow-y:hidden}.screen-legend{bottom:12px;left:12px;right:12px;flex-wrap:wrap}.empty-message{left:50%;top:65%}}@media print{.project-sidebar,.screen-legend{display:none}}
    </style>
</head>
<body>
    ...
    <script>
        ...
        function cardHtml(project) { return `<button class="public-project-card" type="button" data-id="${project.id}">${sliderHtml(project, 148)}<span class="public-card-body"><span class="tag ${slug(project.estado)}">${project.estado}</span><span class="public-card-title">${project.nombre}</span><span class="public-card-meta"><b>Etapa:</b> ${project.etapa}<br><b>Sector:</b> ${project.sector || '-'}<br><b>Monto:</b> ${project.monto || '-'}<br><b>Financiamiento:</b> ${project.financiamiento || '-'}</span><span class="progress"><span style="width:${Number(project.avance) || 0}%"></span></span></span></button>`; }
        ...
        if (bounds.length) {
            map.fitBounds(bounds, {paddingTopLeft: [390, 60], paddingBottomRight: [60, 60], maxZoom: 15});
            focus(projects[0]);
        }
