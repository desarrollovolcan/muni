...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        .public-project-card{background:rgba(255,255,255,.82);border:1px solid var(--line);border-radius:9px;box-shadow:none;cursor:pointer;display:grid;grid-template-columns:82px minmax(0,1fr);min-height:92px;overflow:hidden;text-align:left;transition:border-color .18s ease,background .18s ease,box-shadow .18s ease;width:100%}
        ...
        .public-project-card .map-slider img{border-radius:9px 0 0 9px;height:100%!important;margin:0;min-height:92px}
        .public-card-body{align-content:center;display:grid;min-width:0;padding:9px 10px}
        .public-card-title{color:#334155;display:block;font-size:12px;font-weight:500;line-height:1.38;margin:6px 0 0}
...
<body>
    ...
    <script>
        ...
        function cardHtml(project) { return `<article class="public-project-card" role="button" tabindex="0" data-id="${project.id}">${sliderHtml(project, 110)}<div class="public-card-body"><span class="tag ${slug(project.estado)}">${escapeHtml(project.estado)}</span><strong class="public-card-title">${escapeHtml(project.nombre)}</strong></div></article>`; }
