...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verify_csrf($_POST['csrf_token'] ?? null)) {
    ...
    if ($action === 'delete' && $id > 0) {
        $stmt = db()->prepare('DELETE FROM project_statuses WHERE id = ?');
        $stmt->execute([$id]);
        redirect('estados-proyectos.php?success=1');
    }
    ...
    if ($nombre === '') {
        $errors[] = 'El nombre del estado es obligatorio.';
    }

    if (!$errors) {
        if ($action === 'update' && $id > 0) {
            $stmt = db()->prepare('UPDATE project_statuses SET nombre = ?, orden = ?, activo = ? WHERE id = ?');
            $stmt->execute([$nombre, $orden, $activo, $id]);
        } else {
            $stmt = db()->prepare('INSERT INTO project_statuses (nombre, orden, activo) VALUES (?, ?, ?)');
...
$items = db()->query('SELECT id, nombre, orden, activo FROM project_statuses ORDER BY activo DESC, orden ASC, nombre ASC')->fetchAll();
?>
<?php include('partials/html.php'); ?>
<head>
    <?php $title = 'Estados de proyectos'; include('partials/title-meta.php'); ?>
...
<div class="wrapper">
    ...
    <div class="content-page">
        <div class="container-fluid">
            <?php $subtitle = 'Mantenedores'; $title = 'Estados de proyectos'; include('partials/page-title.php'); ?>
            <?php if ($success === '1') : ?><div class="alert alert-success">Estados actualizados correctamente.</div><?php endif; ?>
            ...
            <div class="row">
                <div class="col-lg-4">
                    <div class="card"><div class="card-header"><h5 class="card-title mb-0">Crear estado</h5></div><div class="card-body">
                        <form method="post">
                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8'); ?>">
                            <input type="hidden" name="action" value="create">
                            <div class="mb-3"><label class="form-label" for="nombre">Nombre</label><input id="nombre" name="nombre" class="form-control" placeholder="Ej: En ejecución" required></div>
                        ...
                        <?php foreach ($items as $item) : ?>
                            <tr><form method="post">
                                ...
                                <td class="text-end">
                                    ...
                                    <button class="btn btn-sm btn-outline-danger" name="action" value="delete" onclick="return confirm('¿Eliminar estado?')">Eliminar</button>
                                </td>
                            </form></tr>
                        <?php endforeach; ?>
